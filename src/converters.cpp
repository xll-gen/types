...
flatbuffers::Offset<protocol::Scalar> ConvertScalar(const XLOPER12& cell, flatbuffers::FlatBufferBuilder& builder) {
    if (cell.xltype == xltypeNum) {
        return protocol::CreateScalar(builder, protocol::ScalarValue::Num,
                                      protocol::CreateNum(builder, cell.val.num).Union());
    } else if (cell.xltype == xltypeInt) {
        return protocol::CreateScalar(builder, protocol::ScalarValue::Int,
                                      protocol::CreateInt(builder, cell.val.w).Union());
    } else if (cell.xltype == xltypeBool) {
        return protocol::CreateScalar(builder, protocol::ScalarValue::Bool,
                                      protocol::CreateBool(builder, cell.val.xbool).Union());
    } else if (cell.xltype == xltypeStr) {
        return protocol::CreateScalar(
            builder, protocol::ScalarValue::Str,
            protocol::CreateStr(builder, builder.CreateString(ConvertExcelString(cell.val.str))).Union());
    } else if (cell.xltype == xltypeErr) {
        return protocol::CreateScalar(builder, protocol::ScalarValue::Err,
                                      protocol::CreateErr(builder, (protocol::XlError)cell.val.err).Union());
    } else {
...
flatbuffers::Offset<protocol::Any> ConvertAny(LPXLOPER12 op, flatbuffers::FlatBufferBuilder& builder) {
    if (op->xltype == xltypeNum) {
        ...
        return protocol::CreateAny(
            builder, protocol::AnyValue::Str,
            protocol::CreateStr(builder, builder.CreateString(ConvertExcelString(op->val.str))).Union());
