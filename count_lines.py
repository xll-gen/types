...
def is_generated(filepath):
    ...
    if norm_path.endswith("include/types/protocol_generated.h"):
        return True

    # 2. Exclude generated Go package for protocol (except manual files)
    if "go/protocol" in norm_path and filepath.endswith(".go"):
        filename = os.path.basename(filepath)
        manual_files = {"extensions.go", "extensions_test.go", "doc.go"}
...
def main():
    ...
    config_files = {
        'cmakelists.txt', 'taskfile.yml', 'go.mod', 'go.sum',
        '.gitignore', '.clang-format', 'cmakepresets.json', 'version'
    }
    ...
    for root, dirs, files in os.walk("."):
        ...
        if ".jules" in dirs:
            dirs.remove(".jules")
        if "build" in dirs: # Exclude build directory
             dirs.remove("build")
        ...
        for file in files:
            ...
            elif filename in config_files or ext in ['.yaml', '.yml', '.mod', '.sum', '.cmake']:
