...
# Define the FlatBuffers version tag
set(FB_VER_TAG "v25.9.23")

# 1. Try to find local flatc in the project root
if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/flatc")
    set(FLATC_EXECUTABLE "${CMAKE_CURRENT_SOURCE_DIR}/flatc")
    message(STATUS "Found local flatc in project root: ${FLATC_EXECUTABLE}")
elseif(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/flatc.exe")
    set(FLATC_EXECUTABLE "${CMAKE_CURRENT_SOURCE_DIR}/flatc.exe")
    message(STATUS "Found local flatc in project root: ${FLATC_EXECUTABLE}")
else()
    # 2. Try to find installed flatc in PATH
    find_program(FLATC_EXECUTABLE flatc)
endif()

if(WIN32 AND NOT FLATC_EXECUTABLE)
    # 3. If not found, try to use a cached downloaded binary (Windows only for now)
    ...
    if(EXISTS "${CACHED_FLATC_PATH}")
        ...
        # 4. If not cached, download pre-built binary
