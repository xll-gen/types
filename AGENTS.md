...
* **DONE (v0.2.5):** C++ `win_compat.h` — now gated behind `XLLGEN_TYPES_TESTING`. A non-Windows production include without the define triggers a `#error` with a clear migration hint pointing at `<windows.h>`. (v0.2.6 follow-up: top-level `CMakeLists.txt` propagates the define to the library and its consumers on non-Windows hosts, so Linux CI still builds after the gate landed.)
