...
* **Deep copy performance**: `deepcopy.go` round-trips through a fresh `flatbuffers.Builder` per call. Pool the builders (`sync.Pool` of `*flatbuffers.Builder` with `Reset()`) for the hot paths in `xll-gen/pkg/server/handlers.go`.
* **Regenerate provenance**: there is no top-level note recording which `flatc` version produced the generated `.go` files. Add a `// Code generated by flatc vX.Y.Z; DO NOT EDIT.` banner check in CI (compare embedded version against `xll-gen/internal/versions/`).
* **C++ `win_compat.h`**: usable for non-Windows compile-tests but easy to misuse in production. Guard it behind `#ifdef XLLGEN_TYPES_TESTING` so it cannot accidentally ship in a real XLL.
